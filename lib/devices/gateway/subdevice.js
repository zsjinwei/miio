...
module.exports = Thing.type(Parent => class SubDevice extends Parent {
	...
	_report(data) {
		...
		if (this._currentRead) {
			if (this._currentRead.fallBackTimer) {
				clearTimeout(this._currentRead.fallBackTimer);
				this._currentRead.fallBackTimer = null;
			}
	...
	loadProperties(props) {
		...
		this._currentRead.promise = new Promise((resolve, reject) => {
			 ...
			 */
			this._currentRead.fallBackTimer = setTimeout(() => {
				this.debug('Read via DEV timed out, using fallback API');
				console.warn('Read via DEV timed out, using fallback API');
				this._parent.call('get_device_prop_exp', [
