...
module.exports = class DeveloperApi extends EventEmitter {
	...
	_onMessage(msg) {
		...
		if (data.cmd && data.cmd !== "heartbeat") {
			console.log(new Date() + " [miio] onMessage \n", data);
		}
			...
			case "report":
				{
					...
					const parsed = data.params; //JSON.parse(data.data);
					...
					this.emit("propertiesChanged", propData);

					this.emit("properties:" + data.sid, propData);
				}
				break;
			default:
